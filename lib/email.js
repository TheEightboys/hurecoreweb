 ...
 * @param {string} jobRole - Job role (e.g., Nurse, Doctor)
 */
async function sendStaffInviteEmail(to, staffName, clinicName, inviteUrl, jobRole = 'Employee') {
  const subject = `You're Invited to Join ${clinicName} on HURE`;

  const content = `
    <h2>You're Invited!</h2>
    <p>Hello ${staffName || 'there'},</p>
    <p>You've been invited to join <strong>${clinicName}</strong> on HURE.</p>
    <div class="info-box">
      <p><strong>Name:</strong> ${staffName || 'Not specified'}</p>
      <p><strong>Email:</strong> ${to}</p>
      <p><strong>Role:</strong> ${jobRole || 'Employee'}</p>
    </div>
    <p style="text-align: center;">
      <a href="${inviteUrl}" class="btn">Accept Invitation</a>
    </p>
    <p class="note">This invitation link expires in 7 days.</p>
    <p class="note">If the button doesn't work, copy this link: ${inviteUrl}</p>
  `;
...
/**
 * Send verification status notification email
 * @param {string} to - Recipient email
 * @param {string} clinicName - Clinic name
 * @param {string} verificationType - 'organization' or 'facility'
 * @param {string} status - 'approved' or 'rejected'
 * @param {string} locationName - Location name (for facility verification)
 * @param {string} reason - Rejection reason (optional)
 */
async function sendVerificationStatusEmail(to, clinicName, verificationType, status, locationName = null, reason = null) {
  const isApproved = status === 'approved';
  const typeLabel = verificationType === 'organization' ? 'Organization' : 'Facility';
  const locationText = locationName ? ` for ${locationName}` : '';

  const subject = isApproved
    ? `✅ Your ${typeLabel} Verification${locationText} is Approved - HURE`
    : `❌ Your ${typeLabel} Verification${locationText} Requires Updates - HURE`;

  const statusBadge = isApproved
    ? '<span style="background-color: #dcfce7; color: #166534; padding: 4px 12px; border-radius: 20px; font-size: 14px;">✓ Approved</span>'
    : '<span style="background-color: #fee2e2; color: #991b1b; padding: 4px 12px; border-radius: 20px; font-size: 14px;">✗ Rejected</span>';

  const content = isApproved ? `
    <h2>${typeLabel} Verification Approved! 🎉</h2>
    <p>Great news! Your ${typeLabel.toLowerCase()} verification${locationText} for <strong>${clinicName}</strong> has been approved by the HURE team.</p>
    <div style="text-align: center; margin: 24px 0;">
      ${statusBadge}
    </div>
    <p>You are now fully verified and can access all features of the HURE platform.</p>
    <p style="text-align: center;">
      <a href="${process.env.APP_URL || 'http://localhost:5173'}/employer" class="btn">Go to Dashboard</a>
    </p>
    <p class="note">Thank you for completing your verification with HURE.</p>
  ` : `
    <h2>${typeLabel} Verification Needs Updates</h2>
    <p>Your ${typeLabel.toLowerCase()} verification${locationText} for <strong>${clinicName}</strong> requires some updates before it can be approved.</p>
    <div style="text-align: center; margin: 24px 0;">
      ${statusBadge}
    </div>
    ${reason ? `<p><strong>Reason:</strong> ${reason}</p>` : ''}
    <p>Please review your submission and update the required information, then submit again for review.</p>
    <p style="text-align: center;">
      <a href="${process.env.APP_URL || 'http://localhost:5173'}/employer" class="btn">Update Verification</a>
    </p>
    <p class="note">If you have questions, please contact our support team.</p>
  `;
